...
#      config.vm.provision "shell", inline: "apt-get -y upgrade"
      ...
      config.vm.provision "shell", inline: "mv /vagrant/"+machine.name+".hosts /etc/hosts"

      if "ubuntu-1" == machine.name then
        ...
        config.vm.provision "shell", inline: "apt-get -y install bash-completion ansible sshpass apt-transport-https ca-certificates curl software-properties-common python-pip"
        config.vm.provision "shell", inline: "apt-add-repository -y ppa:ansible/ansible"
        config.vm.provision "shell", inline: "apt-get update"
        config.vm.provision "shell", inline: "apt-get -y install ansible python-pip"
        config.vm.provision "shell", inline: "mv /etc/ansible/hosts /etc/ansible/hosts.orig"
        config.vm.provision "shell", inline: "mv /vagrant/ansible_inventory /etc/ansible/hosts"

        config.vm.provision "shell", inline: "mkdir -p /etc/ansible/roles/kubernetes"
        config.vm.provision "shell", inline: "cp -R /vagrant/files/ /etc/ansible/roles/kubernetes/"
        config.vm.provision "shell", inline: "cp -R /vagrant/handlers/ /etc/ansible/roles/kubernetes/"
        config.vm.provision "shell", inline: "cp -R /vagrant/tasks/ /etc/ansible/roles/kubernetes/"
        config.vm.provision "shell", inline: "cp -R /vagrant/vars/ /etc/ansible/roles/kubernetes/"
        config.vm.provision "shell", inline: "pip install netaddr"

        config.vm.provision "shell", privileged: false, inline: "cp /vagrant/k8s_install.yaml ~/k8s_install.yaml"
        # make .bashrc pseudo idempotent
        config.vm.provision "shell", privileged: false, inline: "if [ -f ~/.bashrc.orig ]; then cp -v ~/.bashrc.orig ~/.bashrc ;fi"
